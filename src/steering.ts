...
export function resolveSteeringConfig(
  ...
  const row = db.prepare("SELECT value FROM app_settings WHERE key = ?").get(STEERING_URL_SETTING) as { value: string } | undefined;
  if (row) return { ...environment, baseUrl: row.value.trim() || undefined, source: "stored" };
  if (environment.baseUrl?.trim()) return { ...environment, baseUrl: environment.baseUrl.trim(), source: "environment" };
  return { ...environment, baseUrl: undefined, source: "unconfigured" };
}

export function setSteeringUrl(db: Database.Database, value: string): string {
  ensureAppSettings(db);
  const url = value.trim() ? normalizeSteeringUrl(value) : "";
  db.prepare(`
    INSERT INTO app_settings (key, value, updated_at) VALUES (?, ?, ?)
    ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at
  `).run(STEERING_URL_SETTING, url, Date.now());
  return url;
}

export function clearSteeringUrl(db: Database.Database): void {
  ensureAppSettings(db);
  db.prepare("DELETE FROM app_settings WHERE key = ?").run(STEERING_URL_SETTING);
}
...
export async function probeSteeringIntegration(
  ...
  const config = resolveSteeringConfig(db, environment);
  ...
  if (!config.baseUrl) {
    return {
      ...
      status: "unconfigured",
      detail: config.source === "stored" ? "Steering notifications are disabled in Acme Issues." : "No Steering URL is configured.",
      checkedAt,
  ...
  try {
    ...
    const credentialResponse = await fetchFn(`${url}/api/notifications/check`, {
      ...
      body: JSON.stringify({ product: "acme-issues" }),
    ...
    return {
      ...
      detail: online
        ? "Acme Steering is reachable and accepts Issues notifications."
        : `Acme Steering is reachable, but the Issues notification credential was rejected (${credentialResponse.status}).`,
      checkedAt,
      credentialConfigured,
      credentialWillBeSent,
      startupConfigured,
