/**
 * Web UI delivery for the Acme Issues server.
 *
 * Built mode serves the Vite output from dist. Source mode (ACME_ISSUES_DEV=1, set by
 * `npm run dev`) mounts Vite as middleware instead, so editing web/ takes effect over
 * HMR and `acme-issues serve` never needs a prior build.
 */
...
export function webFromSource(): boolean {
  return process.env.ACME_ISSUES_DEV === "1" && existsSync(sourceIndex);
}
...
function viteDevServer(): Promise<ViteDevServer> {
  ...
  // standalone Acme Issues, which would make an embedded Vite proxy back to its own host.
